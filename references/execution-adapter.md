...
生成 Prompt 的人类可读正文统一映射为简体中文；`dialogue_delivery_sheet` 中的最终口播文本和对应字幕文本使用英文。只要存在口播，字幕计划必须是开启状态，样式为白色文字内填充、黑色描边、画面下方安全区。若参考中含公众人物或 IP，传给执行器的 Prompt 只能包含去标识化原创角色描述，不得传递姓名、角色名、品牌标识或直接模仿要求。
